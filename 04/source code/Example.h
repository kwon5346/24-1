...
public:
	Example(HWND window, int width, int height)
	{
		...
		*/
		...
		image.Bloom(0.3f, 300, 1.0f);// image_1
		...
		*/
		...
		/*for (int j = 0; j < image.height; j++)
		{
			for (int i = 0; i < image.width; i++)
			{
				...
				if (i < image.width / 2) {
					image.pixels[idx].v[0] = std::clamp(image.pixels[idx].v[0] * 0.99f, 0.0f, 1.0f);
					image.pixels[idx].v[1] = std::clamp(image.pixels[idx].v[1] * 0.99f, 0.0f, 1.0f);
					image.pixels[idx].v[2] = std::clamp(image.pixels[idx].v[2] * 0.99f, 0.0f, 1.0f);
				}
				else {
					image.pixels[idx].v[0] = std::clamp(image.pixels[idx].v[0] * 1.01f, 0.0f, 1.0f);
					image.pixels[idx].v[1] = std::clamp(image.pixels[idx].v[1] * 1.01f, 0.0f, 1.0f);
					image.pixels[idx].v[2] = std::clamp(image.pixels[idx].v[2] * 1.01f, 0.0f, 1.0f);
				}
			}
		}*/
